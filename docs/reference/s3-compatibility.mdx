...
Fil Hyperspace provides an S3-compatible API. If your application works with AWS S3, it works with Fil Hyperspace — point your SDK or CLI at our endpoint and authenticate with your API keys.
...
Fil Hyperspace supports **AWS Signature Version 4 (SigV4)** authentication. Any S3 client or SDK that supports SigV4 will work without modification.
...
Fil Hyperspace returns standard S3 error XML for all error conditions. If you're handling S3 errors in your application today, no changes are needed.
...
Fil Hyperspace is S3-compatible, not an S3 clone. A few behaviors differ:

| Behavior | AWS S3 | Fil Hyperspace |
|----------|--------|----------------|
...
| Regions | 30+ regions globally. | <!-- PLACEHOLDER: List supported regions --> |
